window.onload = function () {
    ...
    [
      {
        ...
        position: { x: 3, y: 0 },
        direction: "horizontal",
        hint: "A road vehicle, typically with four wheels",
      },
      {
        word: "BIRD",
        position: { x: 3, y: 1 },
        direction: "vertical",
        hint: "A warm-blooded egg-laying vertebrate characterized by feathers",
      },
      {
        word: "BEAR",
        position: { x: 5, y: 1 },
        direction: "horizontal",
        hint: "A large, heavy mammal",
      },
    ],
  ];
  ...
  function create() {
    ...
    this.add
      .text(50, 50, "Score: 0", { font: "20px Arial", fill: "#000000" })
      .setName("scoreText");

    // Create timer display
    timerText = this.add
      .text(50, 80, `Time: ${timer}`, { font: "20px Arial", fill: "#000000" })
      .setName("timerText");
  ...
  function createInputFields(scene, puzzles) {
    ...
    puzzles.forEach((puzzle, index) => {
      ...
      const hintButton = scene.add
        .text(
          startX + scene.cameras.main.width * 0.45,
          startY + 50 * index,
          "Hint",
          { font: "2.5vw Arial", fill: "#0000FF" }
        )
        .setInteractive()
        .on("pointerdown", () => {
      ...
      scene.add.text(
        startX + scene.cameras.main.width * 0.55,
        startY + 50 * index,
        puzzle.word,
